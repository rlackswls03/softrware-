...
현재 PGD는 ε=0.25, 10 steps, random start 1회로 평가하였다. Madry et al.은 PGD와 같은 반복적 first-order adversary를 강건성 평가와 robust optimization의 중요한 기준으로 다루었다[4]. 따라서 본 연구의 PGD-10 결과는 FGSM 방어의 한계를 보여주는 1차 근거이며, 다음 보조 실험을 추가하면 “PGD 설정이 약해서 나온 우연한 결과”라는 질문을 줄일 수 있다.
...
이 추가 검증은 본 연구 범위를 넘어서는 향후 보완으로 남긴다.
