...
선행 연구에서는 이러한 현상을 FGSM adversarial training의 catastrophic overfitting 문제와 연결해 논의한다[5]. 또한 true label을 사용해 FGSM 예제를 생성할 때 모델이 공격 패턴을 역이용하는 label leaking 현상도 알려져 있다[3]. 본 실험 결과만으로 둘 중 하나라고 확정할 수는 없지만, FGSM에는 강하고 PGD에는 취약한 패턴은 두 가능성을 모두 의심하게 만든다.
...
PGD 설정이 약해서 방어 모델이 무너진 것처럼 보였을 가능성을 줄이기 위해, 고정 test subset 2,000개에 대해 PGD-20과 random restart 5회를 추가로 수행하였다. 각 restart에서 생성된 adversarial example 중 target model의 loss가 가장 큰 결과를 샘플별로 선택하였다.

결과는 다음과 같다.

| 모델 | Seed 42 | Seed 123 | Seed 2026 | 평균 ± 표준편차 |
|---|---:|---:|---:|---:|
| LeNet 일반 | 0.15% | 0.35% | 1.20% | 0.57% ± 0.56%p |
| SmallCNN 일반 | 0.35% | 0.80% | 0.05% | 0.40% ± 0.38%p |
| LeNet FGSM 훈련 | 2.05% | 34.40% | 1.75% | 12.73% ± 18.76%p |
| SmallCNN FGSM 훈련 | 16.75% | 0.00% | 0.00% | 5.58% ± 9.67%p |

PGD-20 restart 5회에서도 FGSM 적대적 훈련 모델의 robust 정확도는 낮고 seed별 편차가 크다. 특히 `smallcnn_fgsm_at`은 seed 42에서만 16.75%를 보였고, seed 123과 2026에서는 0%였다. 이는 PGD-10 결과가 단순히 약한 공격 설정 때문에 나온 우연이라고 보기 어렵게 만든다.
...
이 결과는 “공격 강도가 커질수록 더 잘 막았다”고 단순 해석하면 안 된다. 오히려 FGSM 방향이 실제 취약 방향을 제대로 찾지 못하거나, 모델이 특정 FGSM 생성 방식에 과적합했을 가능성을 보여준다. 이런 현상은 gradient masking 또는 obfuscated gradients가 방어 성능을 과대평가하게 만드는 상황과도 연결해 검토할 필요가 있다[6].
...
PGD에서 seed에 따라 거의 무너지는 결과까지 함께 고려하면, FGSM 적대적 훈련 모델에 대해 gradient masking, obfuscated gradients, 또는 공격 방식 과적합 가능성을 추가 분석해야 한다[6].
...
PGD 그래프는 모델별 평균 막대만으로는 seed별 불안정성을 충분히 보여주기 어렵다. 따라서 본 보완에서는 `results/figures/pgd_whitebox.png`를 평균 막대, 표준편차 error bar, seed 42, 123, 2026 개별 점이 함께 보이도록 갱신하였다.

이 수정으로 다음 두 가지가 더 명확해졌다.

- FGSM-AT 모델의 PGD 평균 robust 정확도는 일반 모델보다 높지만, seed별 편차가 매우 크다.
- 특히 LeNet FGSM 훈련과 SmallCNN FGSM 훈련은 표준편차가 평균과 비슷하거나 더 커서, 평균값만으로 안정적인 강건성을 주장하기 어렵다.
...
현재 코드의 `attack_success_rate`는 `1 - robust_accuracy`이므로 clean 이미지부터 틀린 표본도 포함한다. 본 보고서에서는 이를 엄밀한 조건부 공격 성공률로 부르지 않고, 전체 적대적 오분류율에 가까운 값으로 해석한다.
...
Madry et al.은 PGD와 같은 반복적 first-order adversary를 강건성 평가와 robust optimization의 중요한 기준으로 다루었다[4]. 따라서 본 연구의 PGD-10 결과는 FGSM 방어의 한계를 보여주는 1차 근거이며, PGD-20 및 multiple restart는 이 해석을 더 엄밀하게 확인하는 보완 실험이다.

본 보완에서는 다음 설정으로 추가 검증을 수행하였다.
...
추가 실험에서도 FGSM 적대적 훈련 모델은 PGD에 안정적으로 강건하지 않았다. 따라서 “PGD 설정이 약해서 나온 우연한 결과”라는 반론은 상당 부분 줄어든다.
...
추가로 수행한 PGD-20 restart 5회 평가에서도 방어 모델의 robust 정확도는 낮고 seed별 편차가 컸다. 따라서 본 연구의 결론은 PGD-10 단일 평가에만 의존하지 않으며, 더 강한 반복 공격에서도 FGSM 방어의 일반화 한계가 유지된다는 방향으로 강화된다.
