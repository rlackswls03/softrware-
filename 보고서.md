...
PGD 설정이 약해서 방어 모델이 무너진 것처럼 보였을 가능성을 배제하기 위해, 고정 test subset 2,000개에 대해 PGD-20과 random restart 5회를 추가로 수행하였다. 각 restart에서 생성된 adversarial example 중 target model의 loss가 가장 큰 결과를 샘플별로 선택하였다.
...
군 AI 적용 맥락에서, 모든 입력을 무조건 자동 분류하는 것보다 위험 입력을 탐지하고 정화하거나 인간 검토로 넘기는 reject option은 실용적인 방어 절차로 해석될 수 있다.
...
아래 모든 결과는 **seed 42 단일 실험, 전체 테스트 10,000개** 기준이다. Part 1의 FGSM/PGD/전이성 분석은 seed 42, 123, 2026 세 개 seed 평균이지만, Adversarial Firewall 실험은 seed 42 단독으로 수행하였다. 따라서 방화벽 결과는 여러 seed 평균처럼 해석해서는 안 되며, 단일 seed 기준의 추가 시스템 검증으로 해석해야 한다.
...
재구성 오차 기반 탐지는 두 모델 모두에서 FGSM과 PGD 공격을 AUC 1.0, FPR 5% 기준 TPR 100%로 거의 완벽하게 탐지하였다.

### 10.2 정화 및 거부 결과

| 모델 | 조건 | 방화벽 전 정확도 | 정화 후 정확도 | 수락된 표본 정확도 | 탐지율 | 거부율 | 최종 안전 정확도 |
|---|---|---:|---:|---:|---:|---:|---:|
| SmallCNN 일반 | Clean | 99.24% | 98.71% | 99.18% | 3.78% | 0.15% | 99.03% |
| SmallCNN 일반 | FGSM | 22.85% | 68.55% | 75.49% | 100.00% | 21.49% | 80.76% |
| SmallCNN 일반 | PGD | 0.95% | 79.44% | 83.77% | 100.00% | 12.80% | 85.85% |
| SmallCNN FGSM 훈련 | Clean | 99.20% | 98.52% | 99.16% | 3.78% | 0.20% | 98.96% |
| SmallCNN FGSM 훈련 | FGSM | 96.16% | 93.98% | 96.66% | 100.00% | 6.80% | 96.89% |
| SmallCNN FGSM 훈련 | PGD | 29.63% | 81.98% | 86.82% | 100.00% | 13.63% | 88.62% |

주요 결과를 정리하면 다음과 같다.

- `smallcnn_fgsm_at` 기준 PGD 공격 시, 방화벽 적용 전 정확도 29.63%에서 최종 안전 정확도 **88.62%**로 회복되었다.
- `smallcnn_standard` 기준 FGSM 공격 시, 22.85%에서 **80.76%**로 회복되었다.
- `smallcnn_standard` 기준 PGD 공격 시, 0.95%에서 **85.85%**로 회복되었다.
...
- `results/figures/firewall_detection_roc_curve.png` — 재구성 오차 기반 탐지 ROC 곡선. 두 모델 모두 AUC 1.0에 수렴한다.
...
- 모든 방화벽 결과는 seed 42 단일 실험이므로, 여러 seed에 걸친 일반화 성능은 확인되지 않았다.
...
"PGD 설정이 약했기 때문에 나온 결과가 아닌가"라는 반론을 검토하기 위해 PGD-20, random restart 5회로 추가 검증을 수행하였다. 결과는 PGD-10과 동일하게 낮고 seed별 편차가 컸다(SmallCNN AT: 5.58% ± 9.67%p). 이로써 PGD 설정이 약해서 나온 우연이라는 반론은 상당 부분 배제되었다.
...
FGSM AT의 PGD 취약성을 모델 수준에서 해결하는 것에 한계가 있다고 판단하여, 입력 단계에서 적대적 입력을 탐지하고 정화하는 Adversarial Firewall을 구현하였다. Convolutional autoencoder 기반 재구성 오차 탐지와 reject option을 도입한 결과, `smallcnn_fgsm_at` 기준 PGD 공격 시 29.63%에서 최종 안전 정확도 88.62%로 회복되었다.

**→ 방어 파이프라인을 모델 수준에서 시스템 수준으로 확장.**

### 단계별 성능 변화 요약

| 단계 | 핵심 변화 | FGSM ε=0.25 robust | PGD ε=0.25 robust |
|---|---|---:|---:|
| 1단계 | 일반 학습 (SmallCNN 기준) | 28.70% | 0.94% |
| 2단계 | FGSM AT 적용 | **96.65%** ↑ | 10.07% |
| 3단계 | 문제 발견 | 96.65% | **10.07%** (불안정) |
| 4단계 | PGD 강화 검증 | 96.65% | 5.58% (PGD-20 기준) |
| 6단계 | Firewall 추가 (seed 42) | 96.89% | **88.62%** ↑ |
...
> Convolutional autoencoder 기반 재구성 오차 탐지는 FGSM과 PGD 공격을 AUC 1.0 수준으로 탐지할 수 있었으며, 정화와 거부 정책을 통해 공격 상황에서의 최종 안전 정확도를 크게 회복하였다. 다만 이 결과는 seed 42 단일 실험이며, adaptive attack에 대한 보장은 제공하지 않는다.
...
| Autoencoder purifier checkpoint | `checkpoints/autoencoder_seed42.pt` |
| 발표용 시각 보고서 (HTML) | `results/visual_report/index.html` |
| 인터랙티브 방어 시뮬레이션 (HTML) | `results/simulation/index.html` |
| 발표용 인터랙티브 프레젠테이션 (HTML) | `results/presentation/index.html` |
